...
echo "main program not started" > /iexec_out/determinism.txt && echo "init";

echo "{ \"deterministic-output-path\" : \"/iexec_out/determinism.txt\" }" > /iexec_out/computed.json
python main.py $1 -p /iexec_out/ --download=yes
#update status
...
#rm -rf /iexec_out/*.jpg /iexec_out/*.png /iexec_out/*.jpeg  
echo "end program"
